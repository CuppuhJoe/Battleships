...
int runGame(int numGames, Player &player1, Player &player2, int boardSize, string matchFile){
    ...
    bool sr1 = sendReceive(player1, readfds, master_socket, max_sd, client_socket[0], countConnected, gameMsg,
        activity, address, addrlen, client_socket, dConnect,
        c1, valread);
    
    gameMsg.at("str") = to_string(boardSize);

    bool deadGame = false;
    if(!sr1){
        //enter 'this game is over either way' mode
        deadGame = true;
    }
    ...
    bool sr2;
    if(!deadGame){ // why does this if statement work? It's the exact same thing in both the if and the else!
        sr2 = sendReceive(player2, readfds, master_socket, max_sd, client_socket[1], countConnected, gameMsg,
        activity, address, addrlen, client_socket, dConnect,
        c2, valread); 
    }else{
        //this game is dying--see if c2 wins or loses
        sr2 = sendReceive(player2, readfds, master_socket, max_sd, client_socket[1], countConnected, gameMsg,
        activity, address, addrlen, client_socket, dConnect,
        c2, valread); 
    }

    //do error checking, the original
    if(!sr1 && !sr2){
        // return no winner
        cout << "BOTH AI CRASHED!" << endl;
        player1.losses += numGames;
        player2.losses += numGames;
        c1.kill();
        c2.kill();
...
GameInfo runMatch(Player player1, Player player2, int boardSize, int master_socket, int addrlen, int (&client_socket)[30], int activity, int valread, int sd,
    ...
    // if boardSize less then 10 fill board with walls
    if(boardSize < 10){
        for(int row=0;row<10;row++){
            for(int col=0;col<10;col++){
                if(c1Board[row][col]!=WATER){
                    c1Board[row][col] = WALL;
                }
                if(c2Board[row][col]!=WATER){
                    c2Board[row][col] = WALL;
                }
            }
        }
    }
    ...
    while(TRUE){
        ...
        if(totalGameRound <= 6){
            ...
            if(!p1Result && !p2Result){
                // return no winner
                cout << "BOTH AI CRASHED!" << endl;
                temp.player1.losses += numGames - matchNum;
                temp.player2.losses += numGames - matchNum;
                temp.error = true;
                return temp;
            }
                ...
                // return p1 as winner
                cout << "p2 CRASHED!" << endl;
                temp.player1.wins += numGames - matchNum;
                temp.player2.losses += numGames - matchNum;
                temp.error = true;
                return temp;
            }
                ...
                cout << "p1 CRASHED!" << endl;
                temp.player1.losses += numGames - matchNum;
                temp.player2.wins += numGames - matchNum;
                temp.error = true;
                return temp;
            }
            ...
            //do error checking part two, electric boogaloo
            if(!p1Result && !p2Result){
                // return no winner
                cout << "BOTH AI CRASHED!" << endl;
                temp.player1.losses += numGames - matchNum;
            ...
            //do error checking part three, electric beegalee
            if(!p1Result && !p2Result){
                // return no winner
                cout << "BOTH AI CRASHED!" << endl;
                temp.player1.losses += numGames - matchNum;
        ...
        if(totalGameRound > 6 && logging){
            ...
            logAll(10, c1Board, c2Board, player1, player2, log_stream, savedMsg, msg);  // To cheat working this function make boardSize a hardcoded 10
        }
...
bool performAction(string messageType, fd_set &readfds, int &master_socket, int &max_sd, int sd, int &countConnected, json &msg, int (&shipLengths)[6],
    ...
    if ((activity <= 0) && (errno!=EINTR)){
        c.kill();
        // printf("Child disconnected in activity 1\n");
    ...
    if(FD_ISSET(sd, &readfds)){
        ...
        if ((valread = read( sd , buffer, 1499)) <= 0){
            //get the details of the disconnected client and print them
            c.kill();
            // printf("Child disconnected in valread 1\n");
            childDisconnect(sd, address, addrlen, client_socket, dConnect);
            return false;
        }else if(dConnect !=0){
            return true;
...
bool placeShip(char board[10][10], char shipBoard[10][10], int boardSize, int row, int col, int length, Direction dir, json &msg, json (&ships)[6]){
    if( ((row+length>boardSize && dir==VERTICAL) || (col+length>boardSize && dir==HORIZONTAL)) || (row<0 || col<0) ){
        cerr << "Ship placement error!" << endl;
        return false;
    }
    if(dir==HORIZONTAL){
...
bool sendReceive(Player &player, fd_set &readfds, int &master_socket, int &max_sd, int sd, int &countConnected, json &msg,
    ...
    if ((activity <= 0) && (errno!=EINTR)){
        c.kill();
        // printf("Child disconnected in activity 2\n");
        childDisconnect(sd, address, addrlen, client_socket, dConnect);
        return false;
    }
    ...
    if(FD_ISSET(sd, &readfds)){
        ...
        if ((valread = read( sd , buffer, 1499)) <= 0){
            //get the details of the disconnected client and print them
            c.kill();
            // printf("Child disconnected in valread 2\n");
            childDisconnect(sd, address, addrlen, client_socket, dConnect);
            return false;
        }else if(dConnect !=0){
            return true;
